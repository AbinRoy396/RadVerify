...
class ComparisonReport:
    ...
    def format_side_by_side_table(self, ai_findings: Dict[str, Any],
        ...
        # Biometry section
        table.append(f"|{'FETAL BIOMETRY:':<{col_width}}|{'FETAL BIOMETRY:':<{col_width}}|")
    ...
    def format_mismatch_analysis(self, comparisons: Dict[str, Any]) -> str:
        ...
        for param, comp in measurement_comparisons.items():
            if comp['status'] == 'match':
                if comp.get('ai_value') and comp.get('doctor_value'):
                    ...
                    agreements.append(
                        f"   - {param}: Both measured, difference {diff}mm (within tolerance)"
                    )
        ...
        for category, structures in structure_comparisons.items():
            for structure, comp in structures.items():
                if comp['status'] == 'match' and comp.get('ai_present'):
                    agreements.append(
                        f"   - {structure.replace('_', ' ').title()}: Both report present"
                    )

        if agreements:
            sections.append("AGREEMENTS (Items where AI and Doctor agree):")
            sections.extend(agreements[:10])
        ...
        if omissions:
            sections.append("\nOMISSIONS (Items AI detected but Doctor didn't mention):")
            sections.extend(omissions[:5])
            if len(omissions) > 5:
        ...
        if overstatements:
            sections.append("\nOVERSTATEMENTS (Items Doctor mentioned but AI couldn't detect):")
